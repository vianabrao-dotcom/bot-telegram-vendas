...
# =========================
# JOB QUEUE (ex.: tarefas de limpeza/expiração)
# =========================
async def expiration_sweeper(context: ContextTypes.DEFAULT_TYPE):
    """
    Rodará periodicamente.
    Por enquanto só loga para você confirmar que o JobQueue está funcionando.
    (Depois colocamos aqui a lógica de expiração/renovação/remover do grupo.)
    """
    logger.info("expiration_sweeper rodando... %s", datetime.now().isoformat())


async def post_init(app: Application):
    """
    Executa logo após o bot iniciar.
    Aqui registramos o Job repetitivo.
    """
    if app.job_queue is None:
        # Isso NÃO deve mais acontecer com .job_queue(JobQueue())
        logger.error("JobQueue está None. Verifique a criação do Application.")
        return

    # A cada 10 minutos, começando 30s após iniciar
    app.job_queue.run_repeating(expiration_sweeper, interval=600, first=30)
    logger.info("JobQueue OK: expiration_sweeper agendado.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Email do pagador (precisa ser válido).
    # O mais “seguro” aqui é usar um domínio real e um prefixo controlado.
...
def main():
    # ✅ Aqui está o ponto principal da OPÇÃO 1:
    # Criamos e plugamos um JobQueue() no Application
    job_queue = JobQueue()
